...
# replayed domain (13-18), message leaves (19-20), second field (21),
# final screen (22-23). Alpha binds the complete document on replay.
MAIL_CERTIFIED_DECLINABLE = (9, 10, 11, 12, 21)
...
class TestStack07Regressions(common.KeepKeyTest):
    ...
    def test_all_typed_fields_are_reviewed_and_signature_is_unchanged(self):
        ...
        self.assertEqual(buttons, baseline_buttons + 13)  # source/intent, fields, complete document replay
    ...
    def test_typed_replay_change_is_refused(self):
        ...
        assert_failure(self, result, types.Failure_SyntaxError,
                       "EIP-712 replay changed the signed document")
        self.assertEqual(passes, 2)
    ...
    def test_calldata_signing_replay_change_is_refused(self):
        ...
        assert_failure(self, result, types.Failure_SyntaxError,
                       "Signed calldata differs from reviewed calldata")
    ...
    def test_advanced_mode_off_refuses_runtime_signer_preload(self):
        _, program = self._typed_fixture()
        self._load_signer()
        envelope = self._envelope(program)
