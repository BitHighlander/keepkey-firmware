...
TEST(Ethereum, UnknownErc20CannotBePresentedAsAReviewedTransfer) {
  bignum256 amount;
  bn_read_uint32(1, &amount);
  char rendered[32] = {};
  EXPECT_FALSE(ethereumFormatAmount(&amount, UnknownToken, 1, rendered,
                                    sizeof(rendered)));
...
TEST(Ethereum, TransformErc20AlwaysUsesRawCalldataReview) {
  EthereumSignTx msg;
...
TEST(Ethereum, TransformErc20TokenLabelsDoNotBypassRawReview) {
  EthereumSignTx msg;

  // Even known token labels do not describe the dynamic transformations tail.
  MakeTransformErc20(&msg, kTUSD, kTGBP);
  EXPECT_FALSE(
      ethereum_contractHandled(msg.data_initial_chunk.size, &msg, nullptr));
