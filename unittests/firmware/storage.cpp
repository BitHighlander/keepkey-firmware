...
/* v7.15.0-rc17 through -rc26 wrote STORAGE_VERSION 18 before the format was
 * reverted to 17 for release. A device that ran one of those release
 * candidates still has a V18 record on flash.
 *
 * V18 is V17 plus a clear-sign identity block appended after encrypted_sec.
 * Nothing this firmware reads lives in that block, so a V18 record must load
 * as V17 and be rewritten as V17 -- NOT fall through to SUS_Invalid, which
 * calls storage_reset() and destroys the wallet. */
TEST(Storage, V18RecordIsUpgradedRatherThanWiped) {
  ConfigFlash start;
  memset(&start, 0xAB, sizeof(start));
  memcpy(start.meta.magic, "stor", 4);
  start.storage.version = STORAGE_VERSION;
  start.storage.encrypted_sec_version = STORAGE_VERSION;
  start.storage.pub.has_pin = false;
  start.storage.pub.has_wipe_code = false;
  start.storage.pub.has_mnemonic = true;
  start.storage.pub.has_node = false;
  start.storage.pub.policies_count = 1;
  start.storage.has_sec = true;
  start.storage.pub.sca_hardened = false;
  start.storage.pub.v15_16_trans = false;
  ...
  /* Stamp the version word the way those release candidates did. */
  ASSERT_EQ(flash[44], STORAGE_VERSION);
  flash[44] = 18;
  ...
  /* The wallet-loss condition: SUS_Invalid here resets and commits. */
  EXPECT_NE(status, SUS_Invalid)
      << "a V18 record was rejected -- boot would wipe the device";
  EXPECT_EQ(status, SUS_Updated);
  EXPECT_EQ(end.storage.version, STORAGE_VERSION);
}
