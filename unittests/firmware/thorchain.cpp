 ...
 *  so the pin must match on (address, chain_id) together. Before this was
...
static const char *THOR_ETH = "d37bbe5744d730a1d98d8dc97c42f0ca46ad7146";
static const char *THOR_AVAX = "00dc6100103bc402d490aee3f9a5560cbd91f1d4";
...
/* =====================================================================
 *  thor_confirmThorTx — memo length read (no more hardcoded 64).
 * ===================================================================== */
...
static std::vector<uint8_t> build_thor_deposit(const uint8_t vault[20],
  ...
  push_u(0);              // word1: asset = native (zero)
  push_u(1000000000ULL);  // word2: amount
...
// A 67-byte memo (longer than the old hardcoded 64) must now display in full
// through the memo screens, not silently truncate its trailing fields.
TEST(Thorchain, ConfirmThorTxLongMemoDecodesFully) {
  uint8_t vault[20];
  ...
  ASSERT_TRUE(kkconfirm_preload(12, 0));  // generous; extra pairs drain below
