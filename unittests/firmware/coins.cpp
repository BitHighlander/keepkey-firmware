...
TEST(Coins, TableSanity) {
  for (int i = 0; i < COINS_COUNT; ++i) {
    ...
    // Keep this allowlist exact so a newly introduced mismatch cannot hide in
    // it.
    static const char *const kLegacyNoTokenEntry[] = {"ZIL"};
    bool legacy = false;
    for (const char *t : kLegacyNoTokenEntry)
      if (strcmp(coin.coin_shortcut, t) == 0) {
        legacy = true;
        break;
      }
    if (legacy) continue;
...
TEST(Coins, RetiredEthereumContractEntriesStayRemoved) {
  static const char *const kRetired[] = {"QTUM", "BNB", "GTO", "IOST",
                                         "CMT",  "MCO", "ODEM"};
