...
TEST(Eip712, DecimalOverflowRefusedWithoutHashMutation) {
  for (const char* value : {"57896044618658097711785492504343953926634992332820"
                            "282019728792003956564819968",
                            "-5789604461865809771178549250434395392663499233282"
                            "0282019728792003956564819969"}) {
    char types[] = "{\"Review\":[{\"name\":\"value\",\"type\":\"int256\"}]}";
...
TEST(Eip712, NonCanonicalDecimalsRefusedWithoutHashMutation) {
  for (const char* value : {"-0", " -1", "\t-1", "+0"}) {
    ...
    SHA3_CTX actual;
    sha3_256_Init(&actual);
    const SHA3_CTX original = actual;
    ASSERT_TRUE(kkconfirm_preload(0, 1));
    EXPECT_EQ(GENERAL_ERROR, parseVals(t, json_getProperty(t, "Review"),
                                       json_getChild(v), &actual));
    EXPECT_EQ(2, kkconfirm_drain());
    EXPECT_EQ(0, memcmp(&actual, &original, sizeof(actual))) << text;
  }
}
...
TEST(Eip712, IntegerWidthAndValueMustMatchBeforeHashing) {
  ...
  const Case cases[] = {
      ...
      {"int08", "1"},
      {"int256junk", "1"},
      {"uint99999999999999999999", "1"},
  };
  for (const auto& c : cases) {
    SCOPED_TRACE(std::string(c.type) + ":" + c.value);
    std::string types =
        std::string("{\"Review\":[{\"name\":\"value\",\"type\":\"") + c.type +
        "\"}]}";
...
TEST(Eip712, MalformedHexNeverPublishesEncodedOutput) {
  ...
  for (const char* type :
       {"bytes", "bytes0", "bytes33", "bytes256", "bytes01", "bytes1x"}) {
    uint8_t out[32];
    memset(out, 0xa5, sizeof(out));
...
TEST(Eip712, MismatchedJsonShapesAndFixedArraysAreRejectedBeforeHashing) {
  ...
  for (const auto& c : cases) {
    ...
    const SHA3_CTX original = ctx;
    ASSERT_TRUE(kkconfirm_preload(0, 1));
    EXPECT_NE(SUCCESS, parseVals(t, json_getProperty(t, "Review"),
                                 json_getChild(v), &ctx));
    EXPECT_EQ(0, memcmp(&ctx, &original, sizeof(ctx)));
    EXPECT_EQ(2, kkconfirm_drain());
  }
...
TEST(Eip712, MalformedBytesAndAddressesRejectedBeforeAnyValueScreen) {
  ...
  const Case cases[] = {
      ...
      {"address", "\"0xzz\""},
      {"address[]",
       "[\"0x0000000000000000000000000000000000000001\",\"0xzz\"]"},
  };
  for (const auto& c : cases) {
    SCOPED_TRACE(std::string(c.type) + ":" + c.value);
    std::string types =
    ...
    ASSERT_TRUE(kkconfirm_preload(0, 1));
    const int ret =
        parseVals(t, json_getProperty(t, "Review"), json_getChild(v), &ctx);
...
TEST(Eip712, BytesNTypeWidthIsStrictInTypeHashAndEncoder) {
  ...
  for (const char* type :
       {"bytes0", "bytes01", "bytes33", "bytes288", "bytes1x"})
