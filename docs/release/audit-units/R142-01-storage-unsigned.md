...
### R142-02 disposition: local dependency setup, not EVM behavior

The three failures were caused by an uninitialized pinned nested dependency,
`deps/python-keepkey/keepkeylib/eth/ethereum-lists`. The generated table contained
only `#undef X`. Initializing its recorded SHA
`89a64f717e1690bb31adb3e4c38e23640357333c` generated 1,377 lines; rebuilding then
passed all 80 firmware tests. No production EVM or test expectations changed.
Logs: `/private/tmp/7142-complete-deps-rebuild.log` and
`/private/tmp/7142-complete-deps-retests.log`.

A separate build dependency gap required two Ninja invocations after the table
changed. Track that as R142-03 on `audit/7142-token-build-dependencies`, with
explicit generated byproducts. It does not reopen the decoder's local contract.
