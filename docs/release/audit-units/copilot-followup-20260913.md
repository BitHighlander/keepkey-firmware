...
| Copilot 4001507766 P1 ignored protection marker failure | storage.c frozen-head CRC-success branch discards storage_protect_off false; memory.c only returned flash programming status and did not verify bytes. Caller could answer Success while installed bootloader sees missing marker. | FIXED at storage.c 1698 and memory.c 323. Fail closed via shutdown before returning, scrub marshalling buffer and preserve verified active record; verify marker readback. Tests inject explicit write false and successful write with corrupt bytes. Both require exit 1 and an active record surviving exit; no-handler control returns exit 0 and fails both. This does not repair power interruption between active magic and marker. |
