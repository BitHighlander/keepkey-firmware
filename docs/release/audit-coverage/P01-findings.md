...
Local ARM reproduction completed on the history prototype: _ebss=0x2001b7fc and _stack=0x2001f7f8, a 16,380-byte reserve, four below the mandatory floor. recovery_cipher.c's english_alphabet occupied 32 bytes in writable data. All three references were reviewed: initialization and read-only use as strlcpy source/decoded-character lookup; no mutation is required. The one-line const correction is committed as 91439c70f on audit/715-p01-recovery-constant-flash above #738. Rebuild with the same pinned CI image succeeds; Pallas ARM disassembly gate passes. ELF confirms english_alphabet at flash 0x080bde7a (32 bytes), _ebss=0x2001b7dc and _stack=0x2001f7f8: reserve 16,412 bytes, 28 above the unchanged floor. Local ELF SHA-256 e243f5cf4308d2e15e3944feee6c24f4768aab519016a7721e0a5d0f2ac545ad; log /private/tmp/715-constant-flash-arm-build.log. This was a working-tree build immediately before committing the matching source correction, not an exact-head release provenance receipt. Remote CI and broader acceptance remain pending; the narrow margin does not establish worst-case runtime stack usage. 7.14.2/7.14.3 did not fail this reserve gate and are not claimed to require this resource-only correction.
