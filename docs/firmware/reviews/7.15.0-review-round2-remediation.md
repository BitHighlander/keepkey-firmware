...
| B4 | #446 | `recovery_cipher.c:612` guard changes confused strict wordlist recovery with the default import path | ☑ Strict mode rejects non-BIP-39 words and bad checksums; import mode intentionally permits non-word phrases after the independent nonempty/count guard. The default path cannot commit an empty phrase. |
