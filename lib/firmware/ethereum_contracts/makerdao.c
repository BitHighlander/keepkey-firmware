...
static inline bool isMethod(const EthereumSignTx *msg, const char *hash,
                            size_t arg_count)
{
    return hasParams(msg, arg_count) &&
           memcmp(getMethod(msg), hash, 4) == 0;
}
...
bool makerdao_confirmLockAndDraw2(const EthereumSignTx *msg)
{
    ...
    return confirm(ButtonRequestType_ButtonRequest_ConfirmOutput, "MakerDAO",
                   "Create CDP, deposit %s, and withdraw %s from it?",
                   deposit, withdraw);
}
...
bool makerdao_confirmCreateOpenLockAndDraw(const EthereumSignTx *msg)
{
    ...
    return confirm(ButtonRequestType_ButtonRequest_ConfirmOutput, "MakerDAO",
                   "Create Proxy, create CDP, deposit %s, and withdraw %s from it?",
                   deposit, withdraw);
}
...
bool makerdao_confirmLockAndDraw3(const EthereumSignTx *msg)
{
    ...
    return confirm(ButtonRequestType_ButtonRequest_ConfirmOutput, "MakerDAO",
                   "Deposit %s into CDP %" PRIu32 " and withdraw %s?",
                   deposit, cupId, withdraw);
}
...
bool makerdao_confirmMakerDAO(uint32_t data_total, const EthereumSignTx *msg)
{
    ...
    if (makerdao_isClose(msg))
        return makerdao_confirmClose(msg);

    if (makerdao_isGive(msg))
        return makerdao_confirmGive(msg);

    if (makerdao_isLockAndDraw2(msg))
        return makerdao_confirmLockAndDraw2(msg);

    if (makerdao_isCreateOpenLockAndDraw(msg))
        return makerdao_confirmCreateOpenLockAndDraw(msg);

    if (makerdao_isLock(msg))
        return makerdao_confirmLock(msg);

    if (makerdao_isDraw(msg))
        return makerdao_confirmDraw(msg);

    if (makerdao_isLockAndDraw3(msg))
        return makerdao_confirmLockAndDraw3(msg);

    if (makerdao_isFree(msg))
        return makerdao_confirmFree(msg);

    if (makerdao_isWipe(msg))
        return makerdao_confirmWipe(msg);

    if (makerdao_isWipeAndFree(msg))
        return makerdao_confirmWipeAndFree(msg);
