  ...
  for (uint8_t row = 0; row < 3; row++) {
    for (uint8_t col = 0; col < 3; col++) {
      ...
      for (uint8_t color_index = 0;
           color_index < sizeof(color_stepping) / sizeof(color_stepping[0]);
           color_index++) {
        if (cur_pos_elapsed <
...
static void layout_animate_cipher(void* data, uint32_t duration,
  ...
  draw_box_simple(canvas, CIPHER_MASK_COLOR, CIPHER_START_X, 0,
                  KEEPKEY_DISPLAY_WIDTH - CIPHER_START_X,
...
void layout_screen_test(void) {
  draw_box_simple(layout_get_canvas(), TEST_COLOR, TEST_X, TEST_Y, TEST_WIDTH,
                  TEST_HEIGHT);
}
...
void layout_address_notification(const char* desc, const char* address,
  ...
  /* Draw description */
  if (strcmp(desc, "") != 0) {
    const uint32_t title_line_count =
        calc_str_line(address_font, desc, BODY_WIDTH);
...
void layout_address(const char* address, QRSize qr_size) {
  ...
  int y_pos = qr_size == QR_SMALL ? QR_DISPLAY_Y : QR_DISPLAY_Y - 4;

  int side = 0;
  if (qrcodegen_encodeText(
          address, tempdata, codedata, qrcodegen_Ecc_LOW,
          qr_size == QR_SMALL ? qrcodegen_VERSION_MIN : QR_LARGE_VERSION,
          QR_MAX_VERSION, qrcodegen_Mask_AUTO, true)) {
    side = qrcodegen_getSize(codedata);
  }

  // Limit QR to version 1-9
  if (side < 0 || 53 < side) return;
    ...
    for (int j = 0; j < side; j++) {
      if (qrcodegen_getModule(codedata, i, j)) {
        draw_box_simple(
            ...
            QR_DISPLAY_SCALE + (j + y_pos) * QR_DISPLAY_SCALE, QR_DISPLAY_SCALE,
            QR_DISPLAY_SCALE);
      }
    }
  }
