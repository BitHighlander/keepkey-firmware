  ...
  for (uint16_t i = 0; i < num_instructions; i++) {
    ...
    if (memcmp(pi->program_id, SOL_SYSTEM_PROGRAM, SOL_PUBKEY_SIZE) == 0) {
      ...
      if (data_len >= 4) {
        ...
        if (stake_instr == SOL_STAKE_DELEGATE_IX && num_acct_indices >= 6) {
          ...
          memcpy(pi->extra, instr_data + 4, SOL_PUBKEY_SIZE);
          copy_account(pi->from, tx, acct_indices, num_acct_indices, 0);
          /* Canonical layout: 0 = stake account, 1 = Clock sysvar,
           * 2 = [SIGNER] current authority. Index 1 is the sysvar, not the
           * signer — DEACTIVATE (below) and MERGE already skip it correctly. */
          copy_account(pi->authority, tx, acct_indices, num_acct_indices, 2);
          ...
          copy_account(pi->from, tx, acct_indices, num_acct_indices, 0);
          copy_account(pi->authority, tx, acct_indices, num_acct_indices, 2);
        } else if (stake_instr == SOL_STAKE_MERGE_IX && num_acct_indices >= 5) {
      ...
      if (data_len >= 4) {
        ...
        if (vote_instr == SOL_VOTE_AUTHORIZE_IX && data_len >= 40 &&
          ...
          memcpy(pi->extra, instr_data + 4, SOL_PUBKEY_SIZE);
          copy_account(pi->from, tx, acct_indices, num_acct_indices, 0);
          /* Canonical layout: 0 = vote account, 1 = Clock sysvar,
           * 2 = [SIGNER] current authority. Index 1 is the sysvar, not the
           * signer — WITHDRAW/UPDATE_VALIDATOR (below) already use index 2. */
          copy_account(pi->authority, tx, acct_indices, num_acct_indices, 2);
          pi->extra_u8 = (uint8_t)read_le32(instr_data + 36);
