...
void fsm_msgEosGetPublicKey(const EosGetPublicKey* msg) {
  ...
  if (!eos_getPublicKey(node, curve, msg->kind, resp->wif_public_key,
                        sizeof(resp->wif_public_key))) {
  ...
  if (msg->has_show_display && msg->show_display) {
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_Address, node_str, "%s",
                 resp->wif_public_key)) {
      memzero(node, sizeof(*node));
...
void fsm_msgEosSignTx(const EosSignTx* msg) {
  ...
  eos_signingInit(msg->chain_id.bytes, msg->num_actions, &msg->header, root,
...
void fsm_msgEosTxActionAck(const EosTxActionAck* msg) {
  ...
  int action_count = (int)msg->has_transfer + (int)msg->has_delegate +
                     ...
                     (int)msg->has_buy_ram + (int)msg->has_buy_ram_bytes +
                     (int)msg->has_sell_ram + (int)msg->has_vote_producer +
  ...
  if (eos_hasActionUnknownDataRemaining()) {
    if (!msg->has_unknown) {
      fsm_sendFailure(FailureType_Failure_SyntaxError,
                      "Expected more EOSActionUnknown data chunks");
      eos_signingAbort();
      layoutHome();
      return;
    }
