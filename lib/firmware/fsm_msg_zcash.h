...
void fsm_msgZcashSignPCZT(const ZcashSignPCZT *msg) {
  ...
  const uint8_t *seed __attribute__((unused)) = NULL;
  uint32_t seed_len __attribute__((unused)) = 0;

  /* Use fsm_getDerivedNode to get the root node, which triggers
   * passphrase entry if needed. Then we use the seed directly
   * for ZIP-32 Orchard derivation (which is different from BIP-32). */
  HDNode *node = fsm_getDerivedNode(SECP256K1_NAME, NULL, 0, NULL);
  if (!node) {
    layoutHome();
    return;
  }

  /* Derive Orchard keys via ZIP-32 using the master seed.
   * Note: In a production implementation, we'd need access to the raw
   * BIP-39 seed. For now, we derive from the root node's chain_code
   * and private_key as a seed proxy. This is a simplification -
   * proper implementation needs access to the actual 64-byte seed. */
  uint8_t seed_proxy[64];
  memcpy(seed_proxy, node->private_key, 32);
  memcpy(seed_proxy + 32, node->chain_code, 32);

  if (!zcash_derive_orchard_keys(seed_proxy, 64, account,
                                 &zcash_signing.keys)) {
...
void fsm_msgZcashGetOrchardFVK(const ZcashGetOrchardFVK *msg) {
  ...
  /* Derive Orchard keys via ZIP-32 */
  uint8_t seed_proxy[64];
  memcpy(seed_proxy, node->private_key, 32);
  memcpy(seed_proxy + 32, node->chain_code, 32);

  ZcashOrchardKeys keys;
  if (!zcash_derive_orchard_keys(seed_proxy, 64, account, &keys)) {
    memzero(seed_proxy, sizeof(seed_proxy));
    fsm_sendFailure(FailureType_Failure_Other,
                    _("Orchard key derivation failed"));
    layoutHome();
