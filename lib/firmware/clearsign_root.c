...
/* ── The root public key ─────────────────────────────────────────────
 *
 * 7.16 ALWAYS SHIPS THIS ROOT. There is no build flag and no rootless
 * variant: every 7.16 build -- device, emulator, dylib/DLL, release --
 * compiles in these 33 bytes. Vault enables certified ClearSign from the
 * firmware version alone (>= 7.16.0), so a 7.16 build without the root
 * would refuse every certificate it is sent. The version and the root move
 * together; ClearsignRoot.SevenSixteenAlwaysShipsTheRoot fails the unit
 * suite if they ever come apart.
 *
 * The private half lives on the marked root KeepKey
 * (device 393137350D4736341B003900, m/44'/60'/0'/0/0), generated on that
 * device on 2026-08-21, and has never left it. Replacing the root is a
 * reviewed diff to this array plus a fresh ceremony; the same 33 bytes are
 * pinned by ClearsignRoot.TheRootIsTheMarkedRootKeepKeysKey and by the CI and
 * release byte checks (docs/ClearsignRootCeremony.md 4.4), so the change
...
void clearsign_root_pubkey(uint8_t out[CLEARSIGN_PUBKEY_LEN]) {
  memcpy(out, kk_clearsign_root_pubkey, CLEARSIGN_PUBKEY_LEN);
}
