...
void fsm_msgNanoGetAddress(NanoGetAddress* msg) {
  ...
  if (!nano_get_address(&node->public_key[1], coin->nanoaddr_prefix,
                        strlen(coin->nanoaddr_prefix), address,
                        sizeof(address))) {
  ...
  if (msg->has_show_display && msg->show_display) {
    ...
    bool mismatch =
        nano_path_mismatched(coin, msg->address_n, msg->address_n_count);
...
void fsm_msgNanoSignTx(NanoSignTx* msg) {
  ...
  _Static_assert(sizeof(resp->signature.bytes) >= 64,
                 "Signature field not large enough");
  _Static_assert(sizeof(resp->block_hash.bytes) >= 32,
                 "Block hash field not large enough");

  // Sign hash and return the signature
  node = fsm_getDerivedNode(coin->curve_name, msg->address_n,
                            msg->address_n_count, NULL);
  if (!node) {
    nano_signingAbort();
    fsm_sendFailure(FailureType_Failure_Other, _("Could not derive node"));
