...
bool eos_compileActionDelegate(const EosActionCommon* common,
  ...
  CHECK_PARAM_RET(eos_formatName(action->sender, sender), "Invalid name",
                  false);

  char receiver[EOS_NAME_STR_SIZE];
  CHECK_PARAM_RET(eos_formatName(action->receiver, receiver), "Invalid name",
                  false);
...
bool eos_compileActionRefund(const EosActionCommon* common,
  ...
  CHECK_PARAM_RET(eos_formatName(action->owner, owner), "Invalid name", false);

  if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, "Refund",
               "Do you want reclaim all pending unstaked tokens from your %s "
               "account?",
               owner)) {
    fsm_sendFailure(FailureType_Failure_ActionCancelled, "Action Cancelled");
    eos_signingAbort();
...
bool eos_compileActionBuyRam(const EosActionCommon* common,
  ...
  CHECK_PARAM_RET(eos_formatName(action->payer, payer), "Invalid name", false);
  ...
  if (!confirm(
          ButtonRequestType_ButtonRequest_ConfirmEosAction, "Buy Ram",
          "Using your %s account, buy %s worth of RAM for %s at market price?",
...
bool eos_compileActionBuyRamBytes(const EosActionCommon* common,
  ...
  if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction,
               "Buy Ram Bytes",
               "Using your %s account, buy %" PRIu32 " bytes of RAM for %s?",
...
bool eos_compileActionSellRam(const EosActionCommon* common,
  ...
  CHECK_PARAM_RET(eos_formatName(action->account, account), "Invalid name",
                  false);

  if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, "Sell Ram",
               "Using your %s account, sell %" PRIu64
               " bytes of RAM at market price?",
...
bool eos_compileActionVoteProducer(const EosActionCommon* common,
  ...
  if (action->has_proxy && action->proxy != 0) {
    ...
    CHECK_PARAM_RET(eos_formatName(action->voter, voter), "Invalid name",
                    false);

    char proxy[EOS_NAME_STR_SIZE];
    CHECK_PARAM_RET(eos_formatName(action->proxy, proxy), "Invalid name",
                    false);

    if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction,
                 "Vote Producer",
                 "Using your %s account, vote for %s as your proxy?", voter,
                 proxy)) {
      fsm_sendFailure(FailureType_Failure_ActionCancelled, "Action Cancelled");
      eos_signingAbort();
      layoutHome();
    ...
    const uint8_t pages =
        (uint8_t)((action->producers_count / chunk_size) + 1) + 1;

    uint8_t page_no = 1;
    char title[SMALL_STR_BUF];
    snprintf(title, sizeof(title), "Vote Producer %" PRIu8 "/%" PRIu8, page_no,
             pages);

    if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, title,
                 "Using your %s account, vote for the following producers?",
                 voter)) {
      fsm_sendFailure(FailureType_Failure_ActionCancelled, "Action Cancelled");
      eos_signingAbort();
      layoutHome();
    ...
    for (size_t i = 0; i < action->producers_count; i += chunk_size) {
      ...
      for (size_t p = 0; p < chunk_size && p + i < action->producers_count;
           p++) {
      ...
      snprintf(title, sizeof(title), "Vote Producer %" PRIu8 "/%" PRIu8,
               page_no, pages);
      if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, title,
                   "%s", producers)) {
    ...
    if (!confirm(
            ButtonRequestType_ButtonRequest_ConfirmEosAction, "Vote Producer",
            "Using your %s account, do you want to cancel your vote?", voter)) {
      fsm_sendFailure(FailureType_Failure_ActionCancelled, "Action Cancelled");
      eos_signingAbort();
      layoutHome();
      return false;
...
static bool isStandardAuthorization(const EosAuthorization* auth) {
  ...
  if (auth->keys[0].key.size != 0) return false;
...
static bool confirmStandardAuthorization(const char* title,
  ...
  if ((coin = coinByName("EOS")) &&
      !bip32_node_to_string(node_str, sizeof(node_str), coin,
                            ...
                            /*whole_account=*/false,
                            /*show_addridx=*/true) &&
      !bip32_path_to_string(node_str, sizeof(node_str), auth_key->address_n,
                            auth_key->address_n_count)) {
    memset(node_str, 0, sizeof(node_str));
    fsm_sendFailure(FailureType_Failure_SyntaxError,
                    "Cannot encode derived pubkey");
    eos_signingAbort();
    layoutHome();
    return false;
  }
  ...
  if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, title,
               "Do you want to assign signing auth for\n%s to\n%s?", title,
               node_str)) {
    fsm_sendFailure(FailureType_Failure_ActionCancelled, "Action Cancelled");
    eos_signingAbort();
    layoutHome();
...
static bool confirmArbitraryAuthorization(const char* title,
  ...
  for (size_t i = 0; i < auth->keys_count; i++) {
    ...
    if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, title,
                 "Key #%" PRIu8 ":\n%s\nWeight: %" PRIu16, (uint8_t)(i + 1),
                 pubkey, (uint16_t)auth_key->weight)) {
      fsm_sendFailure(FailureType_Failure_ActionCancelled, "Action Cancelled");
      eos_signingAbort();
      layoutHome();
      return false;
  ...
  for (size_t i = 0; i < auth->accounts_count; i++) {
    CHECK_PARAM_RET(auth->accounts[i].account.has_actor,
                    "Required field missing", false);
    CHECK_PARAM_RET(auth->accounts[i].account.has_permission,
                    "Required field missing", false);
    ...
    CHECK_PARAM_RET(
        eos_formatName(auth->accounts[i].account.permission, permission),
        "Invalid name", false);

    if (!confirm(
            ButtonRequestType_ButtonRequest_ConfirmEosAction, title,
            "Account #%" PRIu8
  ...
  for (size_t i = 0; i < auth->waits_count; i++) {
    CHECK_PARAM_RET(auth->waits[i].has_wait_sec, "Required field missing",
                    false);

    if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, title,
                 "Delay #%" PRIu8
                 ":\nDo you want to require a delay of %" PRIu32
                 "s with weight %" PRIu16 "?",
...
bool eos_compileAuthorization(const char* title, const EosAuthorization* auth) {
  ...
  if (isStandardAuthorization(auth)) {
    if (!confirmStandardAuthorization(title, auth)) return false;
  } else {
    if (!confirmArbitraryAuthorization(title, auth)) return false;
  }

  if (!eos_hashAuthorization(&hasher_preimage, auth)) return false;
...
bool eos_compileActionUpdateAuth(const EosActionCommon* common,
  ...
  CHECK_PARAM_RET(eos_formatName(action->parent, parent), "Invalid name",
                  false);
  ...
  if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, title,
               "Update auth for %s with %s permission and %s parent?", account,
               permission, parent)) {
    fsm_sendFailure(FailureType_Failure_ActionCancelled, "Action Cancelled");
    eos_signingAbort();
    layoutHome();
...
bool eos_compileActionDeleteAuth(const EosActionCommon* common,
  ...
  if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, "Delete Auth",
               "Remove %s permission from %s?", permission, account)) {
    fsm_sendFailure(FailureType_Failure_ActionCancelled, "Action Cancelled");
    eos_signingAbort();
    layoutHome();
    return false;
...
bool eos_compileActionLinkAuth(const EosActionCommon* common,
  ...
  CHECK_PARAM_RET(eos_formatName(action->code, code), "Invalid name", false);

  char type[EOS_NAME_STR_SIZE];
  CHECK_PARAM_RET(eos_formatName(action->type, type), "Invalid name", false);

  char requirement[EOS_NAME_STR_SIZE];
  CHECK_PARAM_RET(eos_formatName(action->requirement, requirement),
                  "Invalid name", false);

  if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, "Link Auth",
               "Grant %s permission for the %s contract to %s@%s?", type, code,
               account, requirement)) {
    fsm_sendFailure(FailureType_Failure_ActionCancelled, "Action Cancelled");
    eos_signingAbort();
    layoutHome();
...
bool eos_compileActionUnlinkAuth(const EosActionCommon* common,
  ...
  if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, "Unlink Auth",
               "Unlink %s from auth to %s for %s?", account, code, type)) {
    fsm_sendFailure(FailureType_Failure_ActionCancelled, "Action Cancelled");
    eos_signingAbort();
    layoutHome();
    return false;
...
bool eos_compileActionNewAccount(const EosActionCommon* common,
  ...
  CHECK_PARAM_RET(eos_formatName(action->creator, creator), "Invalid name",
                  false);

  char name[EOS_NAME_STR_SIZE];
  CHECK_PARAM_RET(eos_formatName(action->name, name), "Invalid name", false);

  if (!confirm(ButtonRequestType_ButtonRequest_ConfirmEosAction, "New Account",
               "Using your %s account, create a new account named %s?", creator,
               name)) {
    fsm_sendFailure(FailureType_Failure_ActionCancelled, "Action Cancelled");
    eos_signingAbort();
    layoutHome();
  ...
  if (!eos_compileAuthorization(title, &action->owner)) return false;

  snprintf(title, sizeof(title), "%s@active", name);
  if (!eos_compileAuthorization(title, &action->active)) return false;
