      ...
      if (wire_type == PB_WT_STRING &&
        ...
        while (substream.bytes_left) {
          if ((size_t)*size + 1 > allocated_size) {
             ...
             * upwards. */
            allocated_size +=
                (substream.bytes_left - 1) / iter->pos->data_size + 1;
...
static bool checkreturn decode_field(pb_istream_t *stream,
  ...
  if (PB_HTYPE(iter->pos->type) == PB_HTYPE_ONEOF) {
    if (!pb_release_union_field(stream, iter)) return false;
  }
#endif
...
static bool checkreturn find_extension_field(pb_field_iter_t *iter) {
  ...
  do {
    if (PB_LTYPE(iter->pos->type) == PB_LTYPE_EXTENSION) return true;
...
static bool checkreturn pb_dec_svarint(pb_istream_t *stream,
  ...
  if (!pb_decode_svarint(stream, &value)) return false;
...
static bool checkreturn pb_dec_bytes(pb_istream_t *stream,
  ...
  if (PB_ATYPE(field->type) == PB_ATYPE_POINTER) {
    ...
    PB_RETURN_ERROR(stream, "no malloc support");
#else
    if (!allocate_field(stream, dest, alloc_size, 1)) return false;
...
static bool checkreturn pb_dec_submessage(pb_istream_t *stream,
  ...
  if (field->ptr == NULL) PB_RETURN_ERROR(stream, "invalid field descriptor");
