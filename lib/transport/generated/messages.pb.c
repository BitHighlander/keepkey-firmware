 ...
 * field descriptors.
 */
PB_STATIC_ASSERT((pb_membersize(Features, coins[0]) < 65536 && pb_membersize(Features, policies[0]) < 65536 && pb_membersize(CoinTable, table[0]) < 65536 && pb_membersize(Entropy, entropy) < 65536 && pb_membersize(PublicKey, node) < 65536 && pb_membersize(GetAddress, multisig) < 65536 && pb_membersize(LoadDevice, node) < 65536 && pb_membersize(SignMessage, message) < 65536 && pb_membersize(VerifyMessage, message) < 65536 && pb_membersize(EncryptMessage, message) < 65536 && pb_membersize(EncryptedMessage, message) < 65536 && pb_membersize(DecryptMessage, message) < 65536 && pb_membersize(DecryptedMessage, message) < 65536 && pb_membersize(CipherKeyValue, value) < 65536 && pb_membersize(CipheredKeyValue, value) < 65536 && pb_membersize(TxRequest, details) < 65536 && pb_membersize(TxRequest, serialized) < 65536 && pb_membersize(TxAck, tx) < 65536 && pb_membersize(RawTxAck, tx) < 65536 && pb_membersize(SignIdentity, identity) < 65536 && pb_membersize(SignIdentity, challenge_hidden) < 65536 && pb_membersize(ApplyPolicies, policy[0]) < 65536 && pb_membersize(FlashWrite, data) < 65536 && pb_membersize(DebugLinkFlashDumpResponse, data) < 65536 && pb_membersize(DebugLinkState, layout) < 65536 && pb_membersize(DebugLinkState, node) < 65536), YOU_MUST_DEFINE_PB_FIELD_32BIT_FOR_MESSAGES_Initialize_GetFeatures_Features_GetCoinTable_CoinTable_ClearSession_ApplySettings_ChangePin_Ping_Success_Failure_ButtonRequest_ButtonAck_PinMatrixRequest_PinMatrixAck_Cancel_PassphraseRequest_PassphraseAck_GetEntropy_Entropy_GetPublicKey_PublicKey_GetAddress_Address_WipeDevice_LoadDevice_ResetDevice_EntropyRequest_EntropyAck_RecoveryDevice_WordRequest_WordAck_CharacterRequest_CharacterAck_SignMessage_VerifyMessage_MessageSignature_EncryptMessage_EncryptedMessage_DecryptMessage_DecryptedMessage_CipherKeyValue_CipheredKeyValue_SignTx_TxRequest_TxAck_RawTxAck_SignIdentity_SignedIdentity_ApplyPolicies_FlashHash_FlashWrite_FlashHashResponse_DebugLinkFlashDump_DebugLinkFlashDumpResponse_SoftReset_FirmwareErase_FirmwareUpload_DebugLinkDecision_DebugLinkGetState_DebugLinkState_DebugLinkStop_DebugLinkLog_DebugLinkFillConfig_ChangeWipeCode)
#endif
