...
void layout_init(Canvas* new_canvas) {
  ...
  post_periodic(&layout_animate_callback, NULL, ANIMATION_PERIOD,
                ANIMATION_PERIOD);
}
...
void layout_simple_message(const char* str) {
  ...
  sp.color = TITLE_COLOR;
  draw_string(canvas, font, upper_str, &sp, KEEPKEY_DISPLAY_WIDTH,
              font_height(font));
...
void layout_version(int32_t major, int32_t minor, int32_t patch) {
  ...
  DrawableParams sp;
  sp.x = KEEPKEY_DISPLAY_WIDTH - calc_str_width(font, version_info) -
         BODY_FONT_LINE_PADDING;
  sp.y = KEEPKEY_DISPLAY_HEIGHT - font_height(font) - BODY_FONT_LINE_PADDING;
  sp.color = 0x22;
  draw_string(canvas, font, version_info, &sp, KEEPKEY_DISPLAY_WIDTH,
              font_height(font));
 ...
 *      none
 */
void layout_home_reversed(void) { layout_home_helper(true); }
...
void layout_animate_images(void* data, uint32_t duration, uint32_t elapsed) {
  ...
  if (frameNum != -1 && frameNum < animation->count) {
    draw_bitmap_mono_rle(
        canvas,
        &animation
             ->frames[(frameNum + animation->count - 1) % animation->count],
        true);
